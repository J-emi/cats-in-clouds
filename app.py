...
def create_app():
    ...
    app.config.update(
        ...
        GOOGLE_STORAGE_FILES_BUCKET=os.environ['BUCKET_ID'],
...
def load_cat(id):
    return files.signed_url(f"cats/{id}.jpg")
...
def index():
    ...
    return render_template('index.html',
                           cat_id=cat_id, cat_url=load_cat(cat_id), user_chonk=user_chonk_message,
...
def utility_processor():
    def load_file(name):
        return files.signed_url(name)
    return dict(load_file=load_file)
